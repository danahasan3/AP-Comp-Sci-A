...
public class Picture extends SimplePicture {
    ...
    public void copy(Picture fromPic,
        ...
        fromRow < fromPixels.length && fromRow < fromEndRow && toRow < toPixels.length;
        fromRow++, toRow++){
            for (int fromCol = fromStartCol, toCol = toStartCol;
            fromCol < fromPixels[0].length && fromCol < fromEndCol && toCol < toPixels[0].length;
            fromCol++, toCol++){
    ...
    public void myCollage() {
        ...
        for (int i = 0; i < 250; i += 50) {
            for (int j = 0; j < 250; j += 50) {
                if((i + j) % 3 == 0){
                    this.copy(flowerNoBlue, i, j);
                }else if((i + j) % 3 == 1){
                    this.copy(flowerNegate, i, j);
                }else{
                    this.copy(flowerGrayscale, i, j);
                }
            }
        }
        this.mirrorVertical();
